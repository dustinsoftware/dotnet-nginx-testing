...
echo "directory,100 connections,500 connections,1000 connections,2000 connections" > $output_csv
...
find . -name "bombardier-container.log" | while read log_file; do
  ...
  if [ ${#counts[@]} -eq 4 ]; then
    ...
    echo "$dir_name,${counts[0]},${counts[1]},${counts[2]},${counts[3]}" >> $output_csv
  fi
done
...
echo "|-----------|-----------------|-----------------|------------------|------------------|" >> $markdown_file
tail -n +2 $output_csv | while IFS=, read -r directory c100 c500 c1000 c2000; do
